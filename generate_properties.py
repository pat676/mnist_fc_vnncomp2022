...
def load_data(data_dir: str = "./tmp", num_imgs: int = 25, random: bool = True) -> tuple:
    ...
    sess1 = onnxrun.InferenceSession("./onnx/mnist-net_256x2.onnx")
    sess2 = onnxrun.InferenceSession("./onnx/mnist-net_256x4.onnx")
    sess3 = onnxrun.InferenceSession("./onnx/mnist-net_256x6.onnx")
...
if __name__ == '__main__':
    ...
    for epsilon in epsilons:
        for img_num in range(num_images):
            ...
            image, label = images[img_num], labels[img_num]
            input_bounds = create_input_bounds(image, epsilon)

            spec_path = f"./vnnlib_properties/prop_{img_num}_{epsilon:.2f}.vnnlib"

            save_vnnlib(input_bounds, label, spec_path)

    create_instances_csv(num_props=num_images)
